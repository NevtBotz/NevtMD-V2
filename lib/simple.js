...
exports.smsg = (conn, m, hasParent) => {
     ...
     */
     m.reply = async (text, chatId, options) => {
    	let pp = await conn.getProfilePicture(m.sender).catch(_ => 'https://telegra.ph/file/778acaaf9c0c5ec6265a3.jpg')
        let { data } = await conn.getFile(pp)
        conn.reply(chatId ? chatId : m.chat, text, m, { contextInfo: { mentionedJid: conn.parseMention(text), externalAdReply: { title: 'ꜱɪᴍᴩʟᴇ ᴡʜᴀᴛꜱᴀᴩᴩ ʙᴏᴛ', body: '© NevtGanz', sourceUrl: 'https://chat.whatsapp.com/JDvHx3x9WM69MdoMrPgGIL', thumbnail: data }}, options })
    }
